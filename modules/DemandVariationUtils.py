...
def display_demand_variation_auto_one(csv_file):
    ...
    ax.bar(df["DEPARTURE"], df["ID"], width=pd.Timedelta(minutes=1), align='center', color='steelblue', edgecolor="black")
...
def display_demand_variation_auto(path="", cols=0):
    ...
    def filename_to_label(label):
        # check
        if not label or label.strip() == "":
            logger.error("Invalid or null label.")
            raise ValueError("Invalid or null label.")
...
def display_origin_map_auto(path="", cols=0, crs="EPSG:4326"):
    ...
    # Préparer l’échelle commune de taille
    all_counts = []
    for file in files:
        ...
        gdf = ori_gdf.groupby("ORIGIN")["ID"].count().reset_index()
        all_counts.extend(gdf["ID"].values)
    
    all_counts = np.array(all_counts)
    min_size, max_size = all_counts.min(), all_counts.max()

    # Palette de couleurs
    cmap = plt.cm.get_cmap("tab10", n)  # palette discrète
    ...
    for i, file in enumerate(files):
        ...
        # Taille proportionnelle commune (normalisation sur min/max global)
        gdf["size"] = 20 + 180 * (gdf["ID"] - min_size) / (max_size - min_size + 1e-9)

        r, c = divmod(i, cols)
        ax = axes[r][c]
        gdf.plot(ax=ax, color=colors[i], edgecolor="black", alpha=0.9, markersize=gdf["size"])
    ...
    plt.suptitle("Demand by Origin", fontsize=14)
    plt.show()
...
def display_destination_map_auto(path="", cols=0, crs="EPSG:4326"):
    ...
    for file in files:
        ...
        gdf = dest_gdf.groupby("DESTINATION")["ID"].count().reset_index()
        all_counts.extend(gdf["ID"].values)
    ...
    plt.suptitle("Demand by Destination", fontsize=14)
    plt.show()
