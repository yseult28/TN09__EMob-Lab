...
class ClusteringSimulation:
    ...
    def extract_event_parameters(self, s: str):
        ...
        event_part = s.split("__")[2].split(".csv")[0]

        event_list = list(map(int, event_part.split("_")))

        self._active_events = [event for bit, event in zip(event_list, self._events_parameters) if bit == 1]

        logger.info(f"Active events : {self._active_events}.")

    def apply_events(self):
        """
        Apply all events in self._active_events.

        No parameters
        """

        # calls each event function

        self.apply_PublicTransportStrike(self._active_events[0])

        logger.info("Events applied.")

    def apply_PublicTransportStrike(self, restricted_means):
        """

        No parameters
        """
        self._avalaible_PublicTransport_means = {"METRO", "BUS","TRAM"}
        if restricted_means != "None":
            restricted_means = restricted_means.split('_')
            restricted_means=set(restricted_means)
            self._avalaible_PublicTransport_means  -= restricted_means
        logger.info("PublicTransportStrike applied.")
