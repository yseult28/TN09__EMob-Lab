...
class NaiveSimulation:
    ...
    def extract_name_event(self, s: str):
        """
        Extracts the list of parameters from the 'events' part of a string
        formatted as:
            'law__event', law : None or type_parameters_ratio_period, event : None or type_parameters_period
            Multiple elements in a section are separated by '-'

        - If events == 'None' → returns None
        - Otherwise → returns a list of parameters (split on '-')
        ...
        # parses the event section
        # expected format : type_parameters_period
        ev_parts = event.split("_")
        if len(ev_parts) < 3:
            return None

        # first element = events's type
        name = ev_parts[0]

        return name
        

    def extract_event_parameters(self, s: str):
        """
        Extracts the list of parameters from the 'events' part of a string
        formatted as:
            'law__event', law : None or type_parameters_ratio_period, event : None or type_parameters_period
        ...
        params_part = ev_parts[1]

        # multiple parameters may be separated by '-'
    ...
    def produce_outputs(self):
        ...
        for i in range(len(self._inputs)):
            ...
            # if event = Strike-PublicTransport, do not add the mobility services concerned
            if self.extract_name_event(str(outdir)) ==  "Strike-PublicTransport":
                available_means = {"METRO", "BUS","TRAM"}
                restricted_means = set(self.extract_event_parameters(str(outdir)))
                available_means -= restricted_means
                ...
                logger.info(f"available means : {available_means}")

                attach_log_file(outdir / 'simulation.log')
                
                for mobility_service in available_means:
                ...
                available_means = list(available_means)
                if len(available_means) > 0 :
                    available_means = [str(x) + "Layer" for x in available_means]
                    mmgraph.connect_inter_layers(available_means, 100)
                ...
                #mmgraph.connect_inter_layers(["BUSLayer", "TRAMLayer", "METROLayer"], 100)
